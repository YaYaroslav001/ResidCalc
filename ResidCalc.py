# Данный скрипт строит графики случайных составляющих для нескольких
...
if os.path.exists("Result/Temperature_Residual_Groups.txt"):
    os.remove("Result/Temperature_Residual_Groups.txt")
...
for x in index_list:
    ...
    dec.columns = ['date', 'residual']
    dec["date"] = elem["date"]
    dec["residual"] = seasonal_decompose(elem["temp_avg"], model='additive', period=365).resid
    if elem["date"].iloc[0] != datetime(1950, 1, 1):    # Если станции не хватает данных до 1950-го года, добавляются пустые строки.
        ...
        missing_df.columns = ['date', 'residual']
        missing_df["date"] = missing
        dec = pd.concat([missing_df, dec])
    plt.xlim([datetime(2000, 1, 1), datetime(2010, 1, 1)])
    plt.plot(dec["date"], dec["residual"], label=x)
    df_cor[x] = pd.Series(dec["residual"].to_numpy())
    a = {'index': x,
         'date': dec["date"],
...
fig = plot.get_figure()
fig.savefig("Result/Temperature_Residual_Correlation.png")
...
for x in idx_list:
    with open("Result/Temperature_Residual_Groups.txt", "a") as myfile:
        myfile.write("Group " + str(x) + ":\n")
    cluster_grouped.get_group(x)["index"].to_csv("Result/Temperature_Residual_Groups.txt", header=None,
                                                 index=None, sep=';', mode='a')
